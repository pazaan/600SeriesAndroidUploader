...
public class PumpHistoryBolus extends RealmObject implements PumpHistoryInterface {
    ...
    public List<NightscoutItem> nightscout(PumpHistorySender pumpHistorySender, String senderID) {
        ...
        if (!PumpHistoryParser.BOLUS_PRESET.BOLUS_PRESET_0.equals(bolusPreset)) {
            notes += String.format("[%s]",
                    pumpHistorySender.senderList(senderID, PumpHistorySender.SENDEROPT.BOLUS_PRESET, bolusPreset - 1));
        }
        ...
        if (PumpHistoryParser.BOLUS_TYPE.NORMAL_BOLUS.equals(bolusType)) {
            ...
            notes += String.format("%s%s %s %s %s",
                    notes.length() == 0 ? "" : " ",
                    FormatKit.getInstance().getString(R.string.Square_Bolus),
            ...
            notes += String.format("%s%s %s:%s %s %s",
                    notes.length() == 0 ? "" : " ",
                    FormatKit.getInstance().getString(R.string.Dual_Bolus),
        ...
        if (normalDelivered && normalProgrammedAmount != normalDeliveredAmount) {
            ...
            // normal programmed
            if (PumpHistoryParser.BOLUS_TYPE.NORMAL_BOLUS.equals(bolusType))
                notes += String.format("%s%s %s",
                        notes.length() == 0 ? "" : " ",
                        FormatKit.getInstance().getString(R.string.Normal_Bolus),
                        FormatKit.getInstance().formatAsInsulin(normalProgrammedAmount));
            ...
            notes += String.format("%s* %s %s %s %s %s %s",
                    notes.length() == 0 ? "" : formatSeperator,
                    FormatKit.getInstance().getString(R.string.cancelled),
                    FormatKit.getInstance().getString(R.string.square),
                    FormatKit.getInstance().getString(R.string.delivered),
            ...
            switch (PumpHistoryParser.BOLUS_SOURCE.convert(bolusSource)) {
                case BOLUS_WIZARD:
                    ...
                    if (bgInput != 0 || correctionEstimate != 0) {
                        notes += String.format("%s%s %s %s (%s~%s %s/%s)",
                                notes.length() == 0 ? "" : formatSeperator,
                                FormatKit.getInstance().getString(R.string.bg),
                                FormatKit.getInstance().formatAsFraction(bgInput, PumpHistoryParser.BG_UNITS.MMOL_L.equals(bgUnits)),
                                FormatKit.getInstance().formatAsInsulin(correctionEstimate),
                                FormatKit.getInstance().formatAsFraction(lowBgTarget, PumpHistoryParser.BG_UNITS.MMOL_L.equals(bgUnits)),
                                FormatKit.getInstance().formatAsFraction(highBgTarget, PumpHistoryParser.BG_UNITS.MMOL_L.equals(bgUnits)),
                                FormatKit.getInstance().formatAsFraction(isf, PumpHistoryParser.BG_UNITS.MMOL_L.equals(bgUnits)),
                                FormatKit.getInstance().getString(R.string.text_insulin_unit));
                    }
                    if (iobAdjustment != 0) {
                        notes += String.format("%s%s %s %s",
                                notes.length() == 0 ? "" : formatSeperator,
                                FormatKit.getInstance().getString(R.string.iob),
                ...
                case CLOSED_LOOP_BG_CORRECTION:
                    notes += String.format("%s%s %s",
                            notes.length() == 0 ? "" : formatSeperator,
                            FormatKit.getInstance().getString(R.string.correction),
                            FormatKit.getInstance().formatAsInsulin(correctionEstimate));
                    break;
        ...
        if (PumpHistoryParser.BOLUS_TYPE.SQUARE_WAVE.equals(bolusType) && carbInput == 0) {
            ...
            TreatmentsEndpoints.Treatment treatment2 = nightscoutItem2.treatment();
            nightscoutItem2.mode(nightscoutItem.getMode());
    ...
    public List<MessageItem> message(PumpHistorySender pumpHistorySender, String senderID) {
        ...
        if (normalDelivered && normalProgrammedAmount != normalDeliveredAmount) {
            key = String.format("BOLUS%08X", normalDeliveredRTC);
            date = normalDeliveredDate;
            title = FormatKit.getInstance().getString(R.string.Bolus);
            message += String.format("%s %s • %s %s %s",
                    FormatKit.getInstance().getString(R.string.Normal),
                    FormatKit.getInstance().formatAsInsulin(normalProgrammedAmount),
                    FormatKit.getInstance().getString(R.string.cancelled),
                    FormatKit.getInstance().getString(R.string.delivered),
                    FormatKit.getInstance().formatAsInsulin(normalDeliveredAmount));
            ...
            key = String.format("BOLUS%08X", squareDeliveredRTC);
            ...
            if (PumpHistoryParser.BOLUS_TYPE.SQUARE_WAVE.equals(bolusType)) {
                ...
                message += String.format("%s %s %s:%s %s %s",
                        FormatKit.getInstance().getString(R.string.Dual),
    ...
    public static void bolus(PumpHistorySender pumpHistorySender, Realm realm, Date eventDate, int eventRTC, int eventOFFSET,
                             byte bolusType,
    ...
    public static void bolusWizardEstimate(PumpHistorySender pumpHistorySender, Realm realm, Date eventDate, int eventRTC, int eventOFFSET,
                                           byte bgUnits,
                                           byte carbUnits,
                                           double bgInput,
                                           double carbInput,
